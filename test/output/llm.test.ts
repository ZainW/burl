...
describe('exportLlmJson', () => {
  ...
  test('detects tail latency issues', () => {
    ...
    expect(parsed.interpretation.issues.some((i: string) => i.includes('p99') || i.includes('tail'))).toBe(true);
  });
  ...
  test('detects server errors', () => {
    ...
    expect(parsed.interpretation.issues.some((i: string) => i.includes('5xx') || i.includes('server'))).toBe(true);
    expect(parsed.interpretation.recommendations.some((r: string) => r.includes('server') || r.includes('5xx'))).toBe(true);
  });
  ...
  test('assesses performance correctly', () => {
    const excellentResult = createMockResult({
      ...
      failedRequests: 0,
      latency: {
        min: 1, max: 50, mean: 10, median: 8, stddev: 5,
        p50: 8, p75: 12, p90: 20, p95: 30, p99: 50, p999: 50,
      },
    });
...
describe('exportLlmMarkdown', () => {
  ...
  test('omits issues section when no issues', () => {
    const result = createMockResult({
      ...
      errors: {},
      latency: {
        min: 5, max: 50, mean: 20, median: 18, stddev: 5,
        p50: 18, p75: 25, p90: 35, p95: 40, p99: 45, p999: 50,
      },
    });
    const md = exportLlmMarkdown(result);
