...
function analyzeResult(result: BenchmarkResult): { issues: string[]; recommendations: string[] } {
  ...
  if (errorRate > 0.01) {
    issues.push(`${result.failedRequests} requests failed (${(errorRate * 100).toFixed(2)}% error rate)`);
  }
...
function assessPerformance(result: BenchmarkResult, successRate: number): 'excellent' | 'good' | 'fair' | 'poor' {
